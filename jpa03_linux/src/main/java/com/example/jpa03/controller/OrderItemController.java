...
import com.example.jpa03.dto.OrderItemDTO;
...
import com.example.jpa03.repository.CartRepository;
import com.example.jpa03.repository.MemberRepository;
import com.example.jpa03.repository.OrderDetailRepository;
import com.example.jpa03.repository.OrderItemRepository;
...
public class OrderItemController {
	...
	public String write(@RequestParam(name = "productCode", defaultValue = "0") long productCode,
			@RequestParam(name = "amount", defaultValue = "0") int amount, Model model, HttpServletRequest request,
			HttpSession session) {
	...
	public String process(@PathVariable(name = "orderIdx") int orderIdx, Model model, HttpSession session) {
	...
	public String detail(@PathVariable(name = "orderIdx") int orderIdx, Model model, HttpSession session) {
		Optional<OrderItem> result = orderItemRepository.findById(orderIdx);
		if (result.isPresent()) {
			OrderItem order = result.get();
			List<OrderDetail> detailList = order.getDetailList();
	...
	@Transactional
	@PostMapping("insert")
	public String insert(OrderItemDTO dto, @RequestParam(name = "produtCode", defaultValue = "0") int productCode,
			@RequestParam(name = "amount", defaultValue = "0") int amount, HttpServletRequest request,
			HttpSession session) {
		System.out.println("넘어오는지 확인 필요");
		String userid = (String) session.getAttribute("userid");
	...
	public String cancel(@PathVariable(name = "orderIdx") int orderIdx,
			@RequestParam(name = "cancelReason") String cancelReason, HttpSession session, Model model) {
		String userid = (String) session.getAttribute("userid");
		Optional<OrderItem> result = orderItemRepository.findById(orderIdx);
	...
	public String cancel_process(@PathVariable(name = "orderIdx") int orderIdx, Model model, HttpSession session) {
		Optional<OrderItem> result = orderItemRepository.findById(orderIdx);
		if (result.isPresent()) {
			OrderItem order = result.get();
